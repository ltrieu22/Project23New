...
# Load dataframe for random value generation
_df_cache = None
...
def generate_random_single_turn_examples(df: pd.DataFrame, parser: Any, num_examples: int, max_results_per_example: int) -> List[Dict[str, Any]]:
    ...
    valid_nums = [k for k in NUM_TO_WORD.keys() if k <= max_results_per_example]
    ...
    while len(examples) < num_examples and attempts < max_attempts:
        ...
        if not results.empty:
            ...
            example = create_single_turn_example(
                ...
                df_filtered=results,
                parser=parser,
                max_results=current_num_results
            )
            
            # chekc for duplicate recipe sets
...
def multi_template_2(df: pd.DataFrame, num_results: int):
    ...
    # random nutrient type and value
    nutrient_choices = [
...
def generate_random_multi_turn_examples(df: pd.DataFrame, parser: Any, num_examples: int, max_results_per_example: int) -> List[Dict[str, Any]]:
    ...
    while len(examples) < num_examples and attempts < max_attempts:
        template_func = random.choice(template_generators)
        conversation, query_func = template_func(df, max_results_per_example)
        results = query_func(df)
        ...
        if not results.empty:
            example = create_multi_turn_example(
                ...
                df_filtered=results,
                parser=parser,
                max_results=max_results_per_example
            )
            
            # check for duplicate recipe sets
            recipe_set = tuple(sorted(example['evidence_ids']))
            if recipe_set not in used_recipe_sets:
                examples.append(example)
                used_recipe_sets.add(recipe_set)
